...
- 12칼럼 그리드 캔버스: 드래그&드롭 배치, 리사이즈, **컴포넌트 본문 아무 곳이나 잡아 재배치**(별도 핸들 불필요 — 움직이지 않고 누르면 선택, 4px 이상 끌면 이동)
...
- **87종 컴포넌트 카탈로그** — 레이아웃 10, 입력 17, 데이터 표시 13(**간트 차트·칸반 보드** 포함), 내비게이션 6, 피드백/오버레이 10, 액션 3, 유틸리티 8(**실시간 채팅** 포함), 통계 차트 20종
