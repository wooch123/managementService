...
- 자동 배치는 **종류별 밴드(페이지 · 컴포넌트 · 엔티티 · 액션)** 순서로 오와 열을 맞춰 놓는다. 가로는 밴드가 왼→오 열이고 전체가 **16:9**, 세로는 밴드가 위→아래 행이고 전체가 **9:16**에 가장 가까워지도록 칸 수를 자동으로 정한다(밀도 기본/밀집은 간격만 조절). **지금 보이는 범위**에만 적용되므로, 페이지를 골라 두면 그 페이지 노드만 재배치된다
