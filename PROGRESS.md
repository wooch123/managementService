...
- **운영 DB에 테스트 잔재 행 1건**: `fa_assignments`에 `far_no='123', assignee='123', due_date='123'`.
  `due_date` 정렬에서 맨 위로 올라와 FA 작업 큐 첫 행을 차지한다. 이번 작업이 만든 것이 아니라
  이전에 화면에서 직접 입력된 것으로 보여 손대지 않았다 — 지울지는 확인이 필요하다.
