...
| 요소 | 무엇을 | 없으면 |
|---|---|---|
| 비칠 것 | 테마 색 빛무리를 뒤에 깔고 `background-attachment: fixed` | 유리가 아니라 그냥 흐린 사각형 |
| 두께 | 위 모서리는 밝게, 아래는 어둡게(inset 그림자 두 겹) | 종이처럼 납작하다 |
| 채도 | `saturate(190%)` | 뿌옇기만 하다 |
| 깊이 | 넓고 옅은 바깥 그림자 | 바닥에 붙어 보인다 |

빛을 **고정**한 것이 핵심이다. 스크롤하면 유리판만 지나가고 빛은 제자리에 있어야 판이
움직이는 것처럼 보인다.
...
스크롤                                       빛 고정, 판만 지나감 (attachment: fixed ×4)
성능                                         유리 카드 13장, 6회 스크롤 리페인트 2ms
