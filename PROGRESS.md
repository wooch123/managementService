...
## 2026-09-01 — 종합 현황의 '화면 바로가기' 제거

```
📊 진행 상황
├ 전체 진척도: 100% (운영 중 · 리비전 #90)
├ 현재 작업: 바로가기 카드 제거 — 완료
├ 이번 작업: 100% (검증·실제 이동 경로 확인)
├ 예상 남은 시간: 0m
└ 리스크: 검증 경고 26건이 새로 뜬다(아래 — 오류는 아니다)
```

맨 아래 `nav-cards` 노드를 걷어냈다(사용자 지정). 카드 13 → 12장, 마지막 카드는 'TAT 임박 List'.

**빼면 경고가 난다는 것을 알고 뺐다.** 그 카드는 원래 구성 검증 때문에 둔 것이었고, 걷어내니
`W-REL-007`(도달할 수 없는 페이지)이 **26건** 생겼다. 규칙(`rules/relation.ts`)은 스펙의
`NAVIGATES` 관계 — 컴포넌트·액션에서 페이지로 이어지는 선 — 만 따라가는데, 사이드바는 페이지
트리에서 자동으로 그려져 그런 선을 남기지 않기 때문이다.

**그래도 걷어낸 채로 둔다.** 사이드바를 펼쳐 세어 보니 아홉 화면 링크가 모두 있어
(`/home/overview` · `intake` · `reball` · `request` · `issues` · `info` · `infra` ·
`visit-stats` · `feedback`) **실제로 막힌 화면은 없고**, 경고이지 오류가 아니라 배포도 막지
않는다. 화면 아래 절반을 링크 목록이 차지하는 값이 그 경고를 없애는 값보다 크다고 보았다.

```
검증        오류 0 · 경고 41 (기존 15 + W-REL-007 26)
화면        '화면 바로가기' 사라짐, 카드 12장, 마지막이 TAT 임박 List
이동 경로    사이드바에서 아홉 화면 링크 확인, /home/feedback 직접 열기 200
```

되돌리려면 `overview()` 맨 끝에 `nav-cards` 노드를 다시 두면 된다.
`pnpm test` 383개 통과, typecheck·lint 무경고.

---

## 2026-09-01 — 카드에 얕은 볼록함, 누적 막대의 색 겹침 제거

```
📊 진행 상황
├ 전체 진척도: 100% (운영 중 · 리비전 #90)
├ 현재 작업: 카드 베벨 + 누적 막대 개선 — 완료
├ 이번 작업: 100% (색 중복 0 확인 · 그림자 값 실측)
├ 예상 남은 시간: 0m
└ 리스크: 없음
```
...
앞서 리퀴드 글라스를 입혔다 되돌렸고, 이번에는 반대쪽으로 갔다(사용자 지정). 조명도 흐림도
반투명도 없다. 볼록해 보이는 데 필요한 것은 사실 선 두 개뿐이다.

| | 값 | 왜 |
|---|---|---|
| 위 안쪽 1px | `--foreground` 8% | 빛이 위에서 온다고 읽히면 면이 앞으로 나온다 |
| 아래 안쪽 1px | `--background` 45% | 반대쪽이 그늘지면 두께가 생긴다 |
| 바깥 그림자 | `--foreground` 10%, 1px | 바닥에서 뗄 만큼만. 키우면 곧 '떠 있는 카드'가 된다 |
| 반지름 | 14 → 16px | |

먹이는 법은 리퀴드 때 배운 그대로다 — 반지름·그림자는 렌더러가 인라인 style로 넣으므로 덮지
않고 `--card-radius`/`--card-shadow`를 이 화면에서만 다시 정의하고, 테두리 색은 Tailwind
유틸리티라 `@layer` 밖에 둔다.

### 누적 막대 — 같은 색이 한 막대에 두 번 나왔다

사용자가 층 사이가 볼록해 보인다고 지적한 것의 원인은 그림자가 아니라 **색 겹침**이었다.
테마가 주는 차트 색은 다섯인데 `(i % 5) + 1`로 돌려 쓰고 있었고, '주간 접수'는 계열이 일곱이라
**6·7번째가 1·2번과 완전히 같은 색**이었다(실측: 색 5종 / 계열 7개 / 중복 2). 한 막대 안에
같은 색 층이 두 번 있으면 그 사이에 낀 층들이 하나의 띠처럼 묶여, 층이 솟은 것처럼 읽힌다.

`src/lib/theme/series-color.ts`를 두어 여섯 번째부터는 같은 색을 **카드 바탕 쪽으로 섞어**
한 단계씩 옅게 만든다. 색상각은 그대로라 계열의 성격은 유지되고 밝기가 달라 구별된다.
바탕으로 `--card`를 쓰는 이유는, 그래야 라이트에서는 밝아지고 다크에서는 어두워져 **두 테마
모두에서** 자연스럽게 옅어지기 때문이다(흰색·검정을 박으면 한쪽에서만 맞는다).

같은 5색 순환을 쓰던 네 곳(누적 막대의 색·범례·`chartConfig`, 통계 화면의 `Cell`)을 모두
이 함수로 모았다. 맨 위 층 모서리도 4 → 2px로 낮췄다 — 값이 작은 층이 맨 위에 올 때 봉긋해져
실제보다 크게 읽혔다.

```
색 중복        2 → 0 (계열 7개: 5색 + 옅은 변형 2)
범례 확인      Write Fail 옅은 보라 · 기타 옅은 초록으로 갈림
카드 실측      blur none · 배경 불투명 · inset 8%/45% · 그림자 10% 1px · 반지름 16px
```
...
## 2026-09-01 — 분석 현황 FAR List를 FAR 한 줄로 압축

```
📊 진행 상황
├ 전체 진척도: 100% (운영 중 · 리비전 #91)
├ 현재 작업: FAR List 압축 — 완료
├ 이번 작업: 100% (개수·필터·정렬 DB 대조)
├ 예상 남은 시간: 0m
└ 리스크: 없음
```

**FAR 하나가 한 줄**이 되도록 `groupBy: 'far_no'` + `count()`로 접었다(사용자 지정).
FA Assign 접수 목록에 이미 쓰던 방식과 같다. 불량 대분류 칸도 목록에서 뺐다.

원장은 행 하나가 sample 하나라, 그냥 늘어놓으면 sample 마흔두 개짜리 FAR이 목록의 마흔두 줄을
차지한다. 여기서 고르는 단위는 FAR이므로 목록만 길어지고 고르는 데 도움이 되지 않는다.
**723행 → 342줄.**

**접기 전에 값이 갈리는 칸이 없는지 세어 봤다.** 남기는 칸(담당자·인계 담당자·고객명·제품명·
마감일)과 필터에 쓰는 `failmode1`까지 **한 FAR 안에서 값이 갈리는 FAR은 0건**이었다 — 접어도
잃는 정보가 없다. sample마다 갈리는 값(Part ID·DRAM 등)은 원래 이 목록에 없고 오른쪽 상세와
아래 분석값 표가 맡는다.

불량 대분류는 **목록에서만** 뺐다. 위의 '불량 대분류' 고르개는 그대로 두었고, 칸을 빼도
필터는 살아 있다(WHERE는 GROUP BY 앞에서 걸린다).

```
머리글        FAR No · Sample 수 · 담당자 · 인계 담당자 · 고객명 · 제품명 · 마감일
              (불량 대분류 없음)
중복 FAR      0 — 한 FAR = 한 줄
Sample 수     KR260001 12 · KR260002 42 · FAR-25-1251 3 … 모두 DB와 일치
필터          ?fm=Data Retention → 첫 3건이 DB의 마감일 오름차순과 같음
              (FAR-25-1176 / FAR-25-1059 / FAR-25-1170, 대상 60 FAR)
검증          오류 0 · 경고 41 (이전과 같음, 새 경고 없음)
```
